...
func _draw() -> void:
	if _state == BossState.TELEGRAPH:
		match _current_pattern():
			Pattern.SLAM:
				draw_circle(_slam_target - global_position, _slam_radius, Color(0.8, 0.3, 1.0, 0.25))
			Pattern.CROWN:
				draw_arc(Vector2.ZERO, _crown_radius, -0.6, 0.6, 24, Color(1, 0.5, 0.15, 0.65), 6.0)
	if _projectile_active:
		draw_circle(_projectile_pos - global_position, 10.0, Color(0.4, 1.0, 0.55, 0.75))
		draw_circle(_projectile_pos - global_position, _spit_radius, Color(0.4, 1.0, 0.55, 0.16))
	if _strike_flash_t > 0.0:
		draw_circle(Vector2.ZERO, 48.0, Color(1.0, 0.95, 0.5, 0.25))
	if debug_draw_hitboxes:
		if _state == BossState.ATTACK and _current_pattern() == Pattern.CHARGE:
			draw_circle(Vector2.ZERO, _dash_range, Color(1.0, 0.2, 0.2, 0.2))
		if _state == BossState.ATTACK and _current_pattern() == Pattern.SLAM:
			draw_circle(Vector2.ZERO, _slam_radius, Color(0.8, 0.3, 1.0, 0.2))
...
func _pattern_charge() -> void:
	...
	while elapsed < dash_duration:
		...
		if not _parried and not _hit_applied and global_position.distance_to(_tank.global_position) <= _dash_range:
			_apply_hit_once(18)
...
func _pattern_slam() -> void:
	...
	while elapsed < jump_duration:
		...
		if t >= 0.88 and not _parried and not _hit_applied and global_position.distance_to(_tank.global_position) <= _slam_radius:
			_apply_hit_once(22)
		await get_tree().physics_frame
		elapsed += 1.0 / 60.0
...
func _pattern_crown() -> void:
	_attack_dir = (_tank.global_position - global_position).normalized()
	await _telegraph(0.65, 0.14, "왕관 내려찍기")
	...
	if not _parried and not _hit_applied and in_range and front_dot >= _crown_dot_threshold:
		_apply_hit_once(30)
