...
func _on_parry_attempted() -> void:
	var result := tank.get_parry_result()
	if result == ParryTypes.Result.FAIL:
		_update_ui("패링 실패")
		return
	boss.register_parry()
	tank.set_invincible(0.45)
	var bonus := _berserker_bonus()
	var link := dealer.trigger_link_skill(result == ParryTypes.Result.PERFECT, bonus)
	if dealer.stunned:
		dealer.on_parry_while_stunned(35.0)
	if result == ParryTypes.Result.PERFECT:
		tank.set_invincible(0.7)
		feedback_label.text = "완벽 패링! %s 강화" % link.name
	else:
		feedback_label.text = "패링 성공! %s" % link.name
...
func _on_link_skill_fired(_name: String, damage: int, _empowered: bool) -> void:
	boss.apply_damage(damage)

func _berserker_bonus() -> float:
	var ratio := float(tank.hp) / float(tank.max_hp)
	if ratio <= 0.10:
		return 1.20
	if ratio <= 0.30:
		return 0.70
